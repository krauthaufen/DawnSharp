...
type AdapterProperties =
    ...
    [<CompilerMessage("internal", 9, IsHidden = true)>]
    member x.Pin<'a>(action : DawnRaw.WGPUAdapterProperties -> 'a) : 'a =
        let x = x
...
type BindGroupDescriptor =
    ...
    [<CompilerMessage("internal", 9, IsHidden = true)>]
    member x.Pin<'a>(action : DawnRaw.WGPUBindGroupDescriptor -> 'a) : 'a =
        let x = x
...
type BindGroupEntry =
    ...
    [<CompilerMessage("internal", 9, IsHidden = true)>]
    member x.Pin<'a>(action : DawnRaw.WGPUBindGroupEntry -> 'a) : 'a =
        let x = x
...
type BindGroupLayoutDescriptor =
    ...
    [<CompilerMessage("internal", 9, IsHidden = true)>]
    member x.Pin<'a>(action : DawnRaw.WGPUBindGroupLayoutDescriptor -> 'a) : 'a =
        let x = x
...
type BindGroupLayoutEntry =
    ...
    [<CompilerMessage("internal", 9, IsHidden = true)>]
    member x.Pin<'a>(action : DawnRaw.WGPUBindGroupLayoutEntry -> 'a) : 'a =
        let x = x
        let mutable native = Unchecked.defaultof<DawnRaw.WGPUBindGroupLayoutEntry>
        native.Binding <- x.Binding
        native.Visibility <- x.Visibility
        native.Type <- x.Type
        native.HasDynamicOffset <- (if x.HasDynamicOffset then 1 else 0)
        native.MinBufferBindingSize <- x.MinBufferBindingSize
        native.Multisampled <- (if x.Multisampled then 1 else 0)
...
type BlendDescriptor =
    ...
    [<CompilerMessage("internal", 9, IsHidden = true)>]
    member x.Pin<'a>(action : DawnRaw.WGPUBlendDescriptor -> 'a) : 'a =
        let x = x
...
type BufferCopyView =
    ...
    [<CompilerMessage("internal", 9, IsHidden = true)>]
    member x.Pin<'a>(action : DawnRaw.WGPUBufferCopyView -> 'a) : 'a =
        let x = x
...
type BufferDescriptor =
    ...
    [<CompilerMessage("internal", 9, IsHidden = true)>]
    member x.Pin<'a>(action : DawnRaw.WGPUBufferDescriptor -> 'a) : 'a =
        let x = x
        ...
        try
            ...
            native.Size <- x.Size
            native.MappedAtCreation <- (if x.MappedAtCreation then 1 else 0)
            native.pNext <- 0n
            action native
        finally
            System.Runtime.InteropServices.Marshal.FreeHGlobal pLabel
...
type Color =
    ...
    [<CompilerMessage("internal", 9, IsHidden = true)>]
    member x.Pin<'a>(action : DawnRaw.WGPUColor -> 'a) : 'a =
        let x = x
...
type ColorStateDescriptor =
    ...
    [<CompilerMessage("internal", 9, IsHidden = true)>]
    member x.Pin<'a>(action : DawnRaw.WGPUColorStateDescriptor -> 'a) : 'a =
        let x = x
...
type CommandBufferDescriptor =
    ...
    [<CompilerMessage("internal", 9, IsHidden = true)>]
    member x.Pin<'a>(action : DawnRaw.WGPUCommandBufferDescriptor -> 'a) : 'a =
        let x = x
...
type CommandEncoderDescriptor =
    ...
    [<CompilerMessage("internal", 9, IsHidden = true)>]
    member x.Pin<'a>(action : DawnRaw.WGPUCommandEncoderDescriptor -> 'a) : 'a =
        let x = x
...
type ComputePassDescriptor =
    ...
    [<CompilerMessage("internal", 9, IsHidden = true)>]
    member x.Pin<'a>(action : DawnRaw.WGPUComputePassDescriptor -> 'a) : 'a =
        let x = x
...
type ComputePipelineDescriptor =
    ...
    [<CompilerMessage("internal", 9, IsHidden = true)>]
    member x.Pin<'a>(action : DawnRaw.WGPUComputePipelineDescriptor -> 'a) : 'a =
        let x = x
...
type DepthStencilStateDescriptor =
    ...
    [<CompilerMessage("internal", 9, IsHidden = true)>]
    member x.Pin<'a>(action : DawnRaw.WGPUDepthStencilStateDescriptor -> 'a) : 'a =
        let x = x
        let mutable native = Unchecked.defaultof<DawnRaw.WGPUDepthStencilStateDescriptor>
        native.Format <- x.Format
        native.DepthWriteEnabled <- (if x.DepthWriteEnabled then 1 else 0)
...
type DeviceProperties =
    ...
    [<CompilerMessage("internal", 9, IsHidden = true)>]
    member x.Pin<'a>(action : DawnRaw.WGPUDeviceProperties -> 'a) : 'a =
        let x = x
        let mutable native = Unchecked.defaultof<DawnRaw.WGPUDeviceProperties>
        native.TextureCompressionBC <- (if x.TextureCompressionBC then 1 else 0)
        native.ShaderFloat16 <- (if x.ShaderFloat16 then 1 else 0)
        native.PipelineStatisticsQuery <- (if x.PipelineStatisticsQuery then 1 else 0)
        native.TimestampQuery <- (if x.TimestampQuery then 1 else 0)
        action native
...
type Extent3D =
    ...
    [<CompilerMessage("internal", 9, IsHidden = true)>]
    member x.Pin<'a>(action : DawnRaw.WGPUExtent3D -> 'a) : 'a =
        let x = x
...
type FenceDescriptor =
    ...
    [<CompilerMessage("internal", 9, IsHidden = true)>]
    member x.Pin<'a>(action : DawnRaw.WGPUFenceDescriptor -> 'a) : 'a =
        let x = x
...
type InstanceDescriptor private () =
    ...
    [<CompilerMessage("internal", 9, IsHidden = true)>]
    member x.Pin<'a>(action : DawnRaw.WGPUInstanceDescriptor -> 'a) : 'a =
        let x = x
...
type Origin3D =
    ...
    [<CompilerMessage("internal", 9, IsHidden = true)>]
    member x.Pin<'a>(action : DawnRaw.WGPUOrigin3D -> 'a) : 'a =
        let x = x
...
type PipelineLayoutDescriptor =
    ...
    [<CompilerMessage("internal", 9, IsHidden = true)>]
    member x.Pin<'a>(action : DawnRaw.WGPUPipelineLayoutDescriptor -> 'a) : 'a =
        let x = x
...
type ProgrammableStageDescriptor =
    ...
    [<CompilerMessage("internal", 9, IsHidden = true)>]
    member x.Pin<'a>(action : DawnRaw.WGPUProgrammableStageDescriptor -> 'a) : 'a =
        let x = x
...
type QuerySetDescriptor =
    ...
    [<CompilerMessage("internal", 9, IsHidden = true)>]
    member x.Pin<'a>(action : DawnRaw.WGPUQuerySetDescriptor -> 'a) : 'a =
        let x = x
...
type RasterizationStateDescriptor =
    ...
    [<CompilerMessage("internal", 9, IsHidden = true)>]
    member x.Pin<'a>(action : DawnRaw.WGPURasterizationStateDescriptor -> 'a) : 'a =
        let x = x
...
type RenderBundleDescriptor =
    ...
    [<CompilerMessage("internal", 9, IsHidden = true)>]
    member x.Pin<'a>(action : DawnRaw.WGPURenderBundleDescriptor -> 'a) : 'a =
        let x = x
...
type RenderBundleEncoderDescriptor =
    ...
    [<CompilerMessage("internal", 9, IsHidden = true)>]
    member x.Pin<'a>(action : DawnRaw.WGPURenderBundleEncoderDescriptor -> 'a) : 'a =
        let x = x
...
type RenderPassColorAttachmentDescriptor =
    ...
    [<CompilerMessage("internal", 9, IsHidden = true)>]
    member x.Pin<'a>(action : DawnRaw.WGPURenderPassColorAttachmentDescriptor -> 'a) : 'a =
        let x = x
...
type RenderPassDepthStencilAttachmentDescriptor =
    ...
    [<CompilerMessage("internal", 9, IsHidden = true)>]
    member x.Pin<'a>(action : DawnRaw.WGPURenderPassDepthStencilAttachmentDescriptor -> 'a) : 'a =
        let x = x
        ...
        native.DepthReadOnly <- (if x.DepthReadOnly then 1 else 0)
        native.StencilLoadOp <- x.StencilLoadOp
        native.StencilStoreOp <- x.StencilStoreOp
        native.ClearStencil <- x.ClearStencil
        native.StencilReadOnly <- (if x.StencilReadOnly then 1 else 0)
        action native
[<Struct>]
type RenderPassDescriptor =
    ...
    [<CompilerMessage("internal", 9, IsHidden = true)>]
    member x.Pin<'a>(action : DawnRaw.WGPURenderPassDescriptor -> 'a) : 'a =
        let x = x
...
type RenderPipelineDescriptor =
    ...
    [<CompilerMessage("internal", 9, IsHidden = true)>]
    member x.Pin<'a>(action : DawnRaw.WGPURenderPipelineDescriptor -> 'a) : 'a =
        let x = x
                                            ...
                                            if i >= a.Length then
                                                ...
                                                let rec pinColorStates (a : array<ColorStateDescriptor>) (p : array<_>) (i : int) =
                                                    if i >= a.Length then
                                                        ...
                                                        native.AlphaToCoverageEnabled <- (if x.AlphaToCoverageEnabled then 1 else 0)
                                                        native.pNext <- 0n
                                                        action native
                                                    else
...
type RenderPipelineDescriptorDummyExtension =
    ...
    [<CompilerMessage("internal", 9, IsHidden = true)>]
    member x.Pin<'a>(action : DawnRaw.WGPURenderPipelineDescriptorDummyExtension -> 'a) : 'a =
        let x = x
...
type SamplerDescriptor =
    ...
    [<CompilerMessage("internal", 9, IsHidden = true)>]
    member x.Pin<'a>(action : DawnRaw.WGPUSamplerDescriptor -> 'a) : 'a =
        let x = x
...
type SamplerDescriptorDummyAnisotropicFiltering =
    ...
    [<CompilerMessage("internal", 9, IsHidden = true)>]
    member x.Pin<'a>(action : DawnRaw.WGPUSamplerDescriptorDummyAnisotropicFiltering -> 'a) : 'a =
        let x = x
...
type ShaderModuleSPIRVDescriptor =
    ...
    [<CompilerMessage("internal", 9, IsHidden = true)>]
    member x.Pin<'a>(action : DawnRaw.WGPUShaderModuleSPIRVDescriptor -> 'a) : 'a =
        let x = x
...
type ShaderModuleWGSLDescriptor =
    ...
    [<CompilerMessage("internal", 9, IsHidden = true)>]
    member x.Pin<'a>(action : DawnRaw.WGPUShaderModuleWGSLDescriptor -> 'a) : 'a =
        let x = x
...
type ShaderModuleDescriptor =
    ...
    [<CompilerMessage("internal", 9, IsHidden = true)>]
    member x.Pin<'a>(action : DawnRaw.WGPUShaderModuleDescriptor -> 'a) : 'a =
        let x = x
...
type StencilStateFaceDescriptor =
    ...
    [<CompilerMessage("internal", 9, IsHidden = true)>]
    member x.Pin<'a>(action : DawnRaw.WGPUStencilStateFaceDescriptor -> 'a) : 'a =
        let x = x
...
type SurfaceDescriptor =
    ...
    [<CompilerMessage("internal", 9, IsHidden = true)>]
    member x.Pin<'a>(action : DawnRaw.WGPUSurfaceDescriptor -> 'a) : 'a =
        let x = x
...
type SurfaceDescriptorFromCanvasHTMLSelector =
    ...
    [<CompilerMessage("internal", 9, IsHidden = true)>]
    member x.Pin<'a>(action : DawnRaw.WGPUSurfaceDescriptorFromCanvasHTMLSelector -> 'a) : 'a =
        let x = x
...
type SurfaceDescriptorFromMetalLayer =
    ...
    [<CompilerMessage("internal", 9, IsHidden = true)>]
    member x.Pin<'a>(action : DawnRaw.WGPUSurfaceDescriptorFromMetalLayer -> 'a) : 'a =
        let x = x
...
type SurfaceDescriptorFromWindowsHWND =
    ...
    [<CompilerMessage("internal", 9, IsHidden = true)>]
    member x.Pin<'a>(action : DawnRaw.WGPUSurfaceDescriptorFromWindowsHWND -> 'a) : 'a =
        let x = x
...
type SurfaceDescriptorFromXlib =
    ...
    [<CompilerMessage("internal", 9, IsHidden = true)>]
    member x.Pin<'a>(action : DawnRaw.WGPUSurfaceDescriptorFromXlib -> 'a) : 'a =
        let x = x
...
type SwapChainDescriptor =
    ...
    [<CompilerMessage("internal", 9, IsHidden = true)>]
    member x.Pin<'a>(action : DawnRaw.WGPUSwapChainDescriptor -> 'a) : 'a =
        let x = x
...
type TextureCopyView =
    ...
    [<CompilerMessage("internal", 9, IsHidden = true)>]
    member x.Pin<'a>(action : DawnRaw.WGPUTextureCopyView -> 'a) : 'a =
        let x = x
...
type TextureDataLayout =
    ...
    [<CompilerMessage("internal", 9, IsHidden = true)>]
    member x.Pin<'a>(action : DawnRaw.WGPUTextureDataLayout -> 'a) : 'a =
        let x = x
...
type TextureDescriptor =
    ...
    [<CompilerMessage("internal", 9, IsHidden = true)>]
    member x.Pin<'a>(action : DawnRaw.WGPUTextureDescriptor -> 'a) : 'a =
        let x = x
...
type TextureViewDescriptor =
    ...
    [<CompilerMessage("internal", 9, IsHidden = true)>]
    member x.Pin<'a>(action : DawnRaw.WGPUTextureViewDescriptor -> 'a) : 'a =
        let x = x
...
type VertexAttributeDescriptor =
    ...
    [<CompilerMessage("internal", 9, IsHidden = true)>]
    member x.Pin<'a>(action : DawnRaw.WGPUVertexAttributeDescriptor -> 'a) : 'a =
        let x = x
...
type VertexBufferLayoutDescriptor =
    ...
    [<CompilerMessage("internal", 9, IsHidden = true)>]
    member x.Pin<'a>(action : DawnRaw.WGPUVertexBufferLayoutDescriptor -> 'a) : 'a =
        let x = x
...
type VertexStateDescriptor =
    ...
    [<CompilerMessage("internal", 9, IsHidden = true)>]
    member x.Pin<'a>(action : DawnRaw.WGPUVertexStateDescriptor -> 'a) : 'a =
        let x = x
...
module DawnRaw =
    ...
    [<DllImport("dawn"); SuppressUnmanagedCodeSecurity>]
    extern CommandBufferHandle wgpuCommandEncoderFinish(CommandEncoderHandle Self, DawnRaw.WGPUCommandBufferDescriptor* Descriptor)
    [<DllImport("dawn"); SuppressUnmanagedCodeSecurity>]
    extern ComputePassEncoderHandle wgpuCommandEncoderBeginComputePass(CommandEncoderHandle Self, DawnRaw.WGPUComputePassDescriptor* Descriptor)
    [<DllImport("dawn"); SuppressUnmanagedCodeSecurity>]
    extern RenderPassEncoderHandle wgpuCommandEncoderBeginRenderPass(CommandEncoderHandle Self, DawnRaw.WGPURenderPassDescriptor* Descriptor)
    [<DllImport("dawn"); SuppressUnmanagedCodeSecurity>]
    ...
    [<DllImport("dawn"); SuppressUnmanagedCodeSecurity>]
    extern void wgpuCommandEncoderCopyBufferToTexture(CommandEncoderHandle Self, DawnRaw.WGPUBufferCopyView* Source, DawnRaw.WGPUTextureCopyView* Destination, DawnRaw.WGPUExtent3D* CopySize)
    [<DllImport("dawn"); SuppressUnmanagedCodeSecurity>]
    extern void wgpuCommandEncoderCopyTextureToBuffer(CommandEncoderHandle Self, DawnRaw.WGPUTextureCopyView* Source, DawnRaw.WGPUBufferCopyView* Destination, DawnRaw.WGPUExtent3D* CopySize)
    [<DllImport("dawn"); SuppressUnmanagedCodeSecurity>]
    extern void wgpuCommandEncoderCopyTextureToTexture(CommandEncoderHandle Self, DawnRaw.WGPUTextureCopyView* Source, DawnRaw.WGPUTextureCopyView* Destination, DawnRaw.WGPUExtent3D* CopySize)
    [<DllImport("dawn"); SuppressUnmanagedCodeSecurity>]
    ...
    [<DllImport("dawn"); SuppressUnmanagedCodeSecurity>]
    extern BindGroupHandle wgpuDeviceCreateBindGroup(DeviceHandle Self, DawnRaw.WGPUBindGroupDescriptor* Descriptor)
    [<DllImport("dawn"); SuppressUnmanagedCodeSecurity>]
    extern BindGroupLayoutHandle wgpuDeviceCreateBindGroupLayout(DeviceHandle Self, DawnRaw.WGPUBindGroupLayoutDescriptor* Descriptor)
    [<DllImport("dawn"); SuppressUnmanagedCodeSecurity>]
    extern BufferHandle wgpuDeviceCreateBuffer(DeviceHandle Self, DawnRaw.WGPUBufferDescriptor* Descriptor)
    [<DllImport("dawn"); SuppressUnmanagedCodeSecurity>]
    extern BufferHandle wgpuDeviceCreateErrorBuffer(DeviceHandle Self)
    [<DllImport("dawn"); SuppressUnmanagedCodeSecurity>]
    extern CommandEncoderHandle wgpuDeviceCreateCommandEncoder(DeviceHandle Self, DawnRaw.WGPUCommandEncoderDescriptor* Descriptor)
    [<DllImport("dawn"); SuppressUnmanagedCodeSecurity>]
    extern ComputePipelineHandle wgpuDeviceCreateComputePipeline(DeviceHandle Self, DawnRaw.WGPUComputePipelineDescriptor* Descriptor)
    [<DllImport("dawn"); SuppressUnmanagedCodeSecurity>]
    extern PipelineLayoutHandle wgpuDeviceCreatePipelineLayout(DeviceHandle Self, DawnRaw.WGPUPipelineLayoutDescriptor* Descriptor)
    [<DllImport("dawn"); SuppressUnmanagedCodeSecurity>]
    extern QuerySetHandle wgpuDeviceCreateQuerySet(DeviceHandle Self, DawnRaw.WGPUQuerySetDescriptor* Descriptor)
    [<DllImport("dawn"); SuppressUnmanagedCodeSecurity>]
    extern RenderBundleEncoderHandle wgpuDeviceCreateRenderBundleEncoder(DeviceHandle Self, DawnRaw.WGPURenderBundleEncoderDescriptor* Descriptor)
    [<DllImport("dawn"); SuppressUnmanagedCodeSecurity>]
    extern RenderPipelineHandle wgpuDeviceCreateRenderPipeline(DeviceHandle Self, DawnRaw.WGPURenderPipelineDescriptor* Descriptor)
    [<DllImport("dawn"); SuppressUnmanagedCodeSecurity>]
    extern SamplerHandle wgpuDeviceCreateSampler(DeviceHandle Self, DawnRaw.WGPUSamplerDescriptor* Descriptor)
    [<DllImport("dawn"); SuppressUnmanagedCodeSecurity>]
    extern ShaderModuleHandle wgpuDeviceCreateShaderModule(DeviceHandle Self, DawnRaw.WGPUShaderModuleDescriptor* Descriptor)
    [<DllImport("dawn"); SuppressUnmanagedCodeSecurity>]
    extern SwapChainHandle wgpuDeviceCreateSwapChain(DeviceHandle Self, SurfaceHandle Surface, DawnRaw.WGPUSwapChainDescriptor* Descriptor)
    [<DllImport("dawn"); SuppressUnmanagedCodeSecurity>]
    extern TextureHandle wgpuDeviceCreateTexture(DeviceHandle Self, DawnRaw.WGPUTextureDescriptor* Descriptor)
    [<DllImport("dawn"); SuppressUnmanagedCodeSecurity>]
    ...
    [<DllImport("dawn"); SuppressUnmanagedCodeSecurity>]
    extern SurfaceHandle wgpuInstanceCreateSurface(InstanceHandle Self, DawnRaw.WGPUSurfaceDescriptor* Descriptor)
    [<DllImport("dawn"); SuppressUnmanagedCodeSecurity>]
    ...
    [<DllImport("dawn"); SuppressUnmanagedCodeSecurity>]
    extern FenceHandle wgpuQueueCreateFence(QueueHandle Self, DawnRaw.WGPUFenceDescriptor* Descriptor)
    [<DllImport("dawn"); SuppressUnmanagedCodeSecurity>]
    extern void wgpuQueueWriteBuffer(QueueHandle Self, BufferHandle Buffer, uint64 BufferOffset, nativeint Data, unativeint Size)
    [<DllImport("dawn"); SuppressUnmanagedCodeSecurity>]
    extern void wgpuQueueWriteTexture(QueueHandle Self, DawnRaw.WGPUTextureCopyView* Destination, nativeint Data, unativeint DataSize, DawnRaw.WGPUTextureDataLayout* DataLayout, DawnRaw.WGPUExtent3D* WriteSize)
    [<DllImport("dawn"); SuppressUnmanagedCodeSecurity>]
    ...
    [<DllImport("dawn"); SuppressUnmanagedCodeSecurity>]
    extern RenderBundleHandle wgpuRenderBundleEncoderFinish(RenderBundleEncoderHandle Self, DawnRaw.WGPURenderBundleDescriptor* Descriptor)
    [<DllImport("dawn"); SuppressUnmanagedCodeSecurity>]
    ...
    [<DllImport("dawn"); SuppressUnmanagedCodeSecurity>]
    extern void wgpuRenderPassEncoderSetBlendColor(RenderPassEncoderHandle Self, DawnRaw.WGPUColor* Color)
    [<DllImport("dawn"); SuppressUnmanagedCodeSecurity>]
    ...
    [<DllImport("dawn"); SuppressUnmanagedCodeSecurity>]
    extern TextureViewHandle wgpuTextureCreateView(TextureHandle Self, DawnRaw.WGPUTextureViewDescriptor* Descriptor)
    [<DllImport("dawn"); SuppressUnmanagedCodeSecurity>]
...
type Buffer(device : Device, handle : BufferHandle) =
    ...
    member x.MapAsync(Mode, Offset, Size, Callback : BufferMapAsyncStatus -> nativeint -> unit) : unit =
        let device = device
        let handle = handle
        let mutable _CallbackGC = Unchecked.defaultof<System.Runtime.InteropServices.GCHandle>
        let _Callback = BufferMapCallback(fun Status Userdata -> Callback Status Userdata; _CallbackGC.Free())
        _CallbackGC <- System.Runtime.InteropServices.GCHandle.Alloc(_Callback)
        DawnRaw.wgpuBufferMapAsync(handle, Mode, Offset, Size, System.Runtime.InteropServices.Marshal.GetFunctionPointerForDelegate _Callback, 0n)
...
type Device(handle : DeviceHandle) =
    ...
    member x.SetUncapturedErrorCallback(Callback : ErrorType -> nativeptr<byte> -> nativeint -> unit) : System.IDisposable =
        let device = x
        let handle = handle
        let _Callback = ErrorCallback(Callback)
        let _CallbackGC = System.Runtime.InteropServices.GCHandle.Alloc(_Callback)
        DawnRaw.wgpuDeviceSetUncapturedErrorCallback(handle, System.Runtime.InteropServices.Marshal.GetFunctionPointerForDelegate _Callback, 0n)
        { new System.IDisposable with member x.Dispose() = _CallbackGC.Free() }
    member x.SetDeviceLostCallback(Callback : nativeptr<byte> -> nativeint -> unit) : System.IDisposable =
        let device = x
        let handle = handle
        let _Callback = DeviceLostCallback(Callback)
        let _CallbackGC = System.Runtime.InteropServices.GCHandle.Alloc(_Callback)
        DawnRaw.wgpuDeviceSetDeviceLostCallback(handle, System.Runtime.InteropServices.Marshal.GetFunctionPointerForDelegate _Callback, 0n)
        { new System.IDisposable with member x.Dispose() = _CallbackGC.Free() }
    member x.PushErrorScope(Filter) : unit =
        let device = x
        let handle = handle
        DawnRaw.wgpuDevicePushErrorScope(handle, Filter)
    member x.PopErrorScope(Callback : ErrorType -> nativeptr<byte> -> nativeint -> unit) : bool =
        let device = x
        let handle = handle
        let mutable _CallbackGC = Unchecked.defaultof<System.Runtime.InteropServices.GCHandle>
        let _Callback = ErrorCallback(fun Type Message Userdata -> Callback Type Message Userdata; _CallbackGC.Free())
        _CallbackGC <- System.Runtime.InteropServices.GCHandle.Alloc(_Callback)
        DawnRaw.wgpuDevicePopErrorScope(handle, System.Runtime.InteropServices.Marshal.GetFunctionPointerForDelegate _Callback, 0n) <> 0
...
type Fence(device : Device, handle : FenceHandle) =
    ...
    member x.OnCompletion(Value, Callback : FenceCompletionStatus -> nativeint -> unit) : unit =
        let device = device
        let handle = handle
        let mutable _CallbackGC = Unchecked.defaultof<System.Runtime.InteropServices.GCHandle>
        let _Callback = FenceOnCompletionCallback(fun Status Userdata -> Callback Status Userdata; _CallbackGC.Free())
        _CallbackGC <- System.Runtime.InteropServices.GCHandle.Alloc(_Callback)
        DawnRaw.wgpuFenceOnCompletion(handle, Value, System.Runtime.InteropServices.Marshal.GetFunctionPointerForDelegate _Callback, 0n)
